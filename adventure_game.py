...
items = []
fairies_spells = ["mushroom", "toad", "lobster", "balloon", "chair"]
...
def intro():
    ...
    print_pause(
        "Rumour has it that a wicked fairie is somewhere around here, and has been terrifying the nearby village...")
...
def house(items):
    if "faerie" in items:
        ...
        print_pause(
            "You knock again and wait a few seconds when the door creaks open by itself.")
        ...
        print_pause("goes")
        print_pause("BLACK")
        print_pause("You wake up some time later and realise, it was all a dream \n"
                    "YOU WIN!")
        game_over()
    elif "torch" in items:
        house_list = ['the wizard scowls at you, points to somewhere behind you and slams the door.', 'a cat jumps out from under the table and runs out of the door between your legs',
                      'you flip the switch to turn on the lights, but nothing happens']
        ...
        print_pause("Nice to meet you " + name +
                    " but there's nothing for you here, the forest is nice for visitors.")
        print_pause(
            "You turn away from the door and notice a torch on the ground beside you.  You bend down to pick it up.  You go back to the field.")
        items.append("torch")
        choose_location()
...
def cave(items):
    ...
    if "torch" and "net" in items:
        print_pause(
            "A bear has made its home in the cave, and snarls at you. You can't go back in")
        choose_location()
    elif "torch" in items:
        print_pause("You enter the cave")
        print_pause("You switch on the torch and the cave comes ALIVE!")
        print_pause(
            "You jump back in fear as a huge cloud of swirling claws come towards you screeching in anger")
        print_pause("But WAIT!...")
        print_pause(
            "It's not a cloud of claws, it's a family of bats that woke up when you turned on the torch")
        print_pause(
            "Then you notice, there on the roof, where the bats were sleeping...")
        print_pause(
            "A glittering silver net, with rubies encrusted in the hilt, hangs from the ceiling.  You pick it up and go back to the field.")
        items.append("net")
        choose_location()
    else:
        print_pause("You enter the cave")
        print_pause(
            "It's too dark to see, you need to find something to help.  You go back to the field.")
        choose_location()
...
def forest(items):
    if "faerie" in items:
        ...
        print_pause("SNAP! \n"
                    "You turn around quickly to see what caused the noise but see nothing")
        ...
        if "fight" in response:
            ...
            print_pause("The fairie is right behind you!\n"
                        "She bites you on the nose and you turn into a " + random.choice(fairies_spells))
            print_pause("YOU LOSE!")
            game_over()
    else:
        print_pause(
            "it's too dark to see anything.... come back if you find something to help")
        choose_location()
...
def fight(items):
    ...
    if "net" in items:
        ...
        print_pause("You try to use the torch to hit her but she's too fast!")
        print_pause(
            "You wait until she's so close that you can feel the air from her tiny wings against your face")
        print_pause("You swing the torch...")
        print_pause("You miss!")
        print_pause(
            "The fairie bites you on the ear.... you turn into a " + random.choice(fairies_spells))
        print_pause("YOU LOSE!")
        game_over()
